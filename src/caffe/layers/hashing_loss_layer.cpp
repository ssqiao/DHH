...
void HashingLossLayer<Dtype>::LayerSetUp(
  ...
  diff_.Reshape(1, bottom[0]->channels(), 1, 1);//store the difference of two data's input
...
void HashingLossLayer<Dtype>::Forward_cpu(
  ...
  const Dtype alpha = 2. * top[0]->cpu_diff()[0] / static_cast<Dtype>(bottom[0]->num() * (bottom[0]->num() - 1));//loss term's normlization's size, unfold the sum of loss func,u can get it
  const Dtype beta = 2. * top[0]->cpu_diff()[0] / static_cast<Dtype>(bottom[0]->num());//regularization term's normalization's size, unfold the sum of regularization func, u can get it
  const int channels = bottom[0]->channels();//the dim of each input data (output of last full connect layer)
  Dtype margin = this->layer_param_.hashing_loss_param().bi_margin();// m in objective func
  Dtype tradeoff = this->layer_param_.hashing_loss_param().tradeoff();//tradeoff in objective func
  Dtype tradeoff2 = this->layer_param_.hashing_loss_param().tradeoff2();
  bool multilabel = this->layer_param_.hashing_loss_param().multi_label();//use for NUS-WIDE database training
  bool sim;//determin if two data have the same label
  Dtype loss(0.0);//calc loss value, tmp
  Dtype reg(0.0);//calc regularization term loss
  Dtype reg2(0.0);
  Dtype data(0.0);//get a dim value from each input data
  Dtype dist_sq(0.0);//calc the norm of a vector
  caffe_set(channels*num, Dtype(0), bout);//init the bout as all zero channels*num matrix
  ...
  for (int i = 0; i < num; ++i) {
	...
	sim = ((static_cast<int>(bottom[1]->cpu_data()[i])) == (static_cast<int>(bottom[1]->cpu_data()[j])));
      ...
      if (sim) {  // similar pairs
        loss += dist_sq;
        // gradient with respect to the first sample, a*x+b*y -> y
	caffe_cpu_axpby(
          channels,
          alpha,
          diff_.cpu_data(),
    ...
    for (int k = 0; k < channels;k++){
      ...
      *(bout + (i*channels) + k) += beta * tradeoff * (((data>=Dtype(1.0))||(data<=Dtype(0.0)&&data>=Dtype(-1.0)))?Dtype(1.0):Dtype(-1.0));
      if (std::abs(data)<1) {
      *(bout + (i*channels) + k) += beta * tradeoff2 * ( data>=Dtype(0.0) ? Dtype(-1.0):Dtype(1.0) );   
      }
      reg2 += std::max(1-std::abs(data),Dtype(0.0));
      data = std::abs(data)-1;
      reg += std::abs(data);
  ...
  loss += 2 * tradeoff * reg /static_cast<Dtype>(bottom[0]->num());
  loss += 2 * tradeoff2 * reg2 / static_cast<Dtype>(bottom[0]->num());
  top[0]->mutable_cpu_data()[0] = loss;// pass loss to loss layer's output
}

template <typename Dtype>// havn't use it, calc the loss and gradient at the same time in forward_cpu func
