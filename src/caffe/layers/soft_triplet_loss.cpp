...
#include "caffe/layers/soft_triplet_loss.hpp"
...
namespace caffe {
  ...
  sim_.Reshape(1,1,bottom[0]->num(), bottom[0]->num());//store the similarity <xi,xj> as supervise info 
  ...
  void SoftTripletLossLayer<Dtype>::Forward_cpu(
    ...
    Dtype margin = this->layer_param_.soft_triplet_loss_param().margin();//triplet margin
	Dtype tradeoff = this->layer_param_.soft_triplet_loss_param().tradeoff();//balance param between discriminative loss and stability loss
	//Dtype tradeoff2 = this->layer_param_.soft_triplet_loss_param().tradeoff2();//balance param between discriminative loss and binary loss
	bool sim;
      ...
      for (int j = i+1; j < num; ++j) {
		...
		*(sim_.mutable_cpu_data()+(j*num+i)) = w_sim;
        ...
        for (int k = 0; k < num; ++k) {//calc the log loss in Jij
	        ...
	        Dtype norm2 = 0, norm2_p = 0, w_sim1 = 0, w_sim2 =0;
			...
			w_sim2 = caffe_cpu_dot(dim_fea,fea_data+(j*dim_fea),fea_data+(k*dim_fea));
			
			single_trip_loss += exp(margin-(1+tradeoff*w_sim1)*norm2);
			single_trip_loss += exp(margin-(1+tradeoff*w_sim2)*norm2_p);    
	    }
		if(single_trip_loss > 0)//make sure it has different class loss 
		   single_trip_loss = log(single_trip_loss);
	   
		single_trip_loss += (1+tradeoff*w_sim)*dist_sq;// add the same class loss in Jij, maybe Jij only has the same class loss 
  ...
  void SoftTripletLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    //int label_dim = bottom[1]->count() / bottom[1]->num();
    Dtype margin = this->layer_param_.soft_triplet_loss_param().margin();//triplet loss margin 
	Dtype tradeoff = this->layer_param_.soft_triplet_loss_param().tradeoff();//balance param between discriminative loss and stability loss 
	//Dtype tradeoff2 = this->layer_param_.soft_triplet_loss_param().tradeoff2();//balance discriminative loss & reg loss 
	bool sim;
	//Dtype data(0.0);// one bit data
    ...
    for (int i = 0; i < num; ++i) {
      ...
      for (int j = i+1; j < num; ++j) {
		...
		caffe_cpu_axpby(dim, alpha*(1+tradeoff*sim_.cpu_data()[i*num+j]), diff_.cpu_data(), Dtype(1.0), diff + (i*dim));
		caffe_cpu_axpby(dim, -alpha*(1+tradeoff*sim_.cpu_data()[i*num+j]), diff_.cpu_data(), Dtype(1.0), diff + (j*dim));
	    ...
	    for (int k = 0; k < num; ++k) {//triplet gradient w.r.t bi, bk, bj, bl
		    ...
		    caffe_cpu_axpby(dim, alpha*deriva_ik*(1+tradeoff*sim_.cpu_data()[i*num+k]), diff_.cpu_data(), Dtype(1.0), diff + (i*dim));
		    caffe_cpu_axpby(dim, -alpha*deriva_ik*(1+tradeoff*sim_.cpu_data()[i*num+k]), diff_.cpu_data(), Dtype(1.0), diff + (k*dim));
			
			caffe_sub(dim, bottom_data+(j*dim), bottom_data+(k*dim), diff_.mutable_cpu_data());//bj-bl
		    caffe_cpu_axpby(dim, alpha*deriva_jk*(1+tradeoff*sim_.cpu_data()[j*num+k]), diff_.cpu_data(), Dtype(1.0), diff + (j*dim)); 
		    caffe_cpu_axpby(dim, -alpha*deriva_jk*(1+tradeoff*sim_.cpu_data()[j*num+k]), diff_.cpu_data(), Dtype(1.0), diff + (k*dim));
