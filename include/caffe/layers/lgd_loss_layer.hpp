#ifndef CAFFE_LGD_LOSS_LAYER_HPP_
#define CAFFE_LGD_LOSS_LAYER_HPP_
...
class LgdLossLayer : public LossLayer<Dtype> {
 public:
  explicit LgdLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff1_(),diff2_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "LgdLoss"; }

 protected:
  /// @copydoc LgdLossLayer
  ...
  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  // the vector distance in the learned Euclidean space
  Blob<Dtype> diff1_;
  Blob<Dtype> diff2_;
  // the real loss (+/- difference) between the logm distance and vector distance of each sample pair
  // Blob<Dtype> realLoss_;
};

}  // namespace caffe

#endif  // CAFFE_LGD_LOSS_LAYER_HPP_
