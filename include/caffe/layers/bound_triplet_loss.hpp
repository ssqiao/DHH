...
class BoundTripletLossLayer : public LossLayer<Dtype> {
 public:
  explicit BoundTripletLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_(), dist_norm(), trip_loss() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "BoundTripletLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return bottom_index != 1;
  }

 protected:
  /// @copydoc BoundTripletLossLabelLayer
  ...
  Blob<Dtype> diff_;
  Blob<Dtype> dist_norm;  
  Blob<Dtype> trip_loss;
  Dtype n_tri;  
 };
