#ifndef CAFFE_CROSS_TRIPLET_LOSS_LAYER_HPP_
#define CAFFE_CROSS_TRIPLET_LOSS_LAYER_HPP_
...
class CrossTripletLossLayer : public LossLayer<Dtype> {
 public:
  explicit CrossTripletLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "CrossTripletLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return bottom_index != 1;
  }

 protected:
  /// @copydoc TripletRankLossLabelLayer
...
#endif  // CAFFE_CROSS_TRIPLET_LOSS_LAYER_HPP_
